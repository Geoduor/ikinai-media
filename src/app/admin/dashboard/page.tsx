...
export default async function DashboardPage() {
  ...
  const [postCount, publishedCount, unreadCount] = await Promise.all([
    prisma.post.count(),
  ...
  return (
    <div>
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-3">
        ...
        <div className="rounded-2xl border border-dashed border-line p-6 text-ink/50">
          <p className="font-mono text-[12px] uppercase tracking-wider">Coming next</p>
          <p className="mt-2 text-sm">Portfolio, Team &amp; Package editors</p>
        </div>
      </div>
    </div>
  );
