...
- **Primary color:** Brand yellow `#FFE218` — measured directly from the real logo file (see Section 6), supersedes the earlier estimate from the profile PDF's cover design.
...
- **Logo:** Real source file received — see Section 6.
...
**Logo:** Real source file received from Builder (Aug 10, 2026) as a high-res PDF
(`ikinai_media_Logo_260810_185907.pdf` — a 300dpi scan/export, not vector). Extracted to
`public/brand/logo-mark.png` (transparent, mark only) and `public/brand/logo-full.png`
(full lockup). Exact brand yellow measured from the file: `#FFE218` — site color tokens
updated to match. Wordmark text in the source file is `#555555` dark gray; the site codes
"IKINAI MEDIA" as text rather than using the raster wordmark, so its color can adapt on
dark backgrounds (footer) — see `src/components/Logo.tsx`.
...
**Portfolio content:** Six real behind-the-scenes production photos added (Sept 2026) from a live studio recording of "Off the Pitch, On the Record," a talk show on Kenya hockey's future, produced at Baraza Media Lab. Image files stored in `public/portfolio/`; captions were written from what's directly visible in the photos (on-set signage, equipment). Entries still need to be created in the Portfolio admin editor (`/admin/portfolio/new`) pointing at these files, since Claude can't write directly to the live production database.

## 7. Still Open
1. Any additional team members beyond the two named in the profile.
2. Final domain name + production Vercel/DB (Supabase or Neon) account access.
3. Migrate Supabase + Vercel project ownership to the client's own accounts before public launch (currently running on the Builder's personal accounts for development/testing).
4. Team/Packages admin editors — schema exists, UI not built yet (Posts, Portfolio, and Contact inbox are self-serve so far).
5. More portfolio media beyond the initial six "Off the Pitch" photos, and video content once available.
